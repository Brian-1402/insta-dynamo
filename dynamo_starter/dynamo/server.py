...
class BayouServer(Server):
  ...
  def __send_anti_entropy(self, peer: str) -> tuple[list[LogEntry], SortedList[LogEntry]]:
    ...
    if self._is_master:
      ...
      #? Following logic correct?
      # Master does not need tentative logs (since it can directly commit the logs) 
      # And logically only master would have latest committed logs, so no need to check for committed logs
      # So, the commited logs that should be returned for master should be the obtained anti entropy tentative logs
      committed_logs = tentative_logs
