...
class AppLogEntry(LogEntry):
    ...
    def do(self, state: State) -> None:
        ...
        tstate.s += self.char # Append the character to the state 
        # self.ltime = LogicalTime(self.ltime.name, time.monotonic_ns()) #? Update the ltime

    def undo(self, state: State) -> None:
        tstate = cast(AppState, state)
        # TODO 2:- Apply this logEntry on the state 
        #? Remove last character or last occurance of self.char from the state?
        if len(tstate.s) > 0:
            tstate.s = tstate.s[:-1] # Remove the last character from the state -> The rollback of tentative logs should be implemented in the reverse order of the sorted tentative logs
        # self.ltime = LogicalTime(self.ltime.name, time.monotonic_ns()) #? Update the ltime
